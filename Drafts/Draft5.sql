...
--
-- Host: 127.0.0.1
-- Generation Time: Oct 25, 2022 at 09:20 PM
...
CREATE DEFINER=`root`@`localhost` PROCEDURE `uspInsertResort` (IN `ResortName` VARCHAR(255), IN `ResortDesc` TEXT, IN `ResortCountryId` INT)   INSERT INTO resort
	(
    	resort.resort_name,
        resort.resort_description,
        resort.resort_country_id
    )
VALUES
	(ResortName, ResortDesc, ResortCountryId)$$
...
ALTER TABLE `resort`
  ...
  ADD KEY `FK__country__country_id_4` (`resort_country_id`);
...
--
ALTER TABLE `resort`
  ADD CONSTRAINT `FK__country__country_id_4` FOREIGN KEY (`resort_country_id`) REFERENCES `country` (`country_id`);
